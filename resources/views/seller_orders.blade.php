                ...
                <div class="float-right">
                    <form method="POST" action="{{ route('seller.updateLineStatus', ['lineId' => $orderLines->first()->id]) }}" class="form-inline">
                        ...
                        <select name="order_status" class="form-control form-control-sm mr-2">
                            <option value="hazırlanıyor" {{ $orderLines->first()->order_status == 'sipari alındı' ? 'selected' : '' }}>Sipariş Alındı</option>
                            ...
                            <tr>
                                <td>
                                    @if(App\Models\Product::where('product_sku', $line->product_sku)->first())
                                        <img src="{{ asset(App\Models\Product::where('product_sku', $line->product_sku)->first()->product_image) }}" class="order_image">
                                    @else
                                        Ürün Bulunamadı.
                                    @endif
                                </td>
